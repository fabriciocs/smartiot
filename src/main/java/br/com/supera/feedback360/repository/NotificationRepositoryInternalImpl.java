...
import br.com.supera.feedback360.repository.rowmapper.NotificationRowMapper;
...
class NotificationRepositoryInternalImpl extends SimpleR2dbcRepository<Notification, Long> implements NotificationRepositoryInternal {
    ...
    private static final Table recipientTable = Table.aliased("sys_user", "recipient");
    ...
    RowsFetchSpec<Notification> createQuery(Pageable pageable, Condition whereClause) {
        ...
        columns.addAll(SysUserSqlHelper.getColumns(recipientTable, "recipient"));
        SelectFromAndJoinCondition selectFrom = Select.builder()
            .select(columns)
            .from(entityTable)
            .leftOuterJoin(recipientTable)
            .on(Column.create("recipient_id", entityTable))
            .equals(Column.create("id", recipientTable));
    ...
    private Notification process(Row row, RowMetadata metadata) {
        ...
        entity.setRecipient(sysuserMapper.apply(row, "recipient"));
        return entity;
    }
