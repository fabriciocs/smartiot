...
/**
 * REST controller for managing {@link br.com.supera.feedback360.domain.SysUser}.
 */
@RestController
...
public class SysUserResource {
     ...
     */
    @PostMapping("")
    public Mono<ResponseEntity<SysUserDTO>> createSysUser(@Valid @RequestBody SysUserDTO sysUserDTO) throws URISyntaxException {
     ...
     */
    @PutMapping("/{id}")
    public Mono<ResponseEntity<SysUserDTO>> updateSysUser(
    ...
    public Mono<ResponseEntity<SysUserDTO>> partialUpdateSysUser(
        @PathVariable(value = "id", required = false) final Long id,
        ...
        return sysUserRepository
            ...
            .flatMap(exists -> {
                ...
                Mono<SysUserDTO> result = sysUserService.partialUpdate(sysUserDTO);
    ...
    public Mono<ResponseEntity<List<SysUserDTO>>> getAllSysUsers(
        ...
        return sysUserService
            .countAll()
            .zipWith(sysUserService.findAll(pageable).collectList())
            .map(
                countWithEntities ->
                    ResponseEntity.ok()
                        .headers(
     ...
     */
    @GetMapping("/{id}")
    public Mono<ResponseEntity<SysUserDTO>> getSysUser(@PathVariable("id") Long id) {
        log.debug("REST request to get SysUser : {}", id);
        Mono<SysUserDTO> sysUserDTO = sysUserService.findOne(id);
     ...
     */
    @DeleteMapping("/{id}")
    public Mono<ResponseEntity<Void>> deleteSysUser(@PathVariable("id") Long id) {
