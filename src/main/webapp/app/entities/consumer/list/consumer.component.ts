...
import { EntityArrayResponseType, ConsumerService } from '../service/consumer.service';
...
export class ConsumerComponent implements OnInit {
  ...
  trackId = (_index: number, item: IConsumer): number => this.consumerService.getConsumerIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.consumers = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IConsumer[] | null): IConsumer[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.consumerService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
