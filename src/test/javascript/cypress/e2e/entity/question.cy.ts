...
describe('Question e2e test', () => {
  ...
  const questionSample = { questionText: 'mutilate average forenenst', questionType: 'though miniaturize crystal' };
  ...
  describe('new Question page', () => {
    ...
    it('should create an instance of Question', () => {
      cy.get(`[data-cy="questionText"]`).type('scarce because nor');
      cy.get(`[data-cy="questionText"]`).should('have.value', 'scarce because nor');

      cy.get(`[data-cy="questionType"]`).type('jovially');
      cy.get(`[data-cy="questionType"]`).should('have.value', 'jovially');
