...
describe('Notification e2e test', () => {
  ...
  const notificationSample = {
    type: 'irritably',
    message: 'primary impel breathe',
    status: 'lazily miscount receive',
    createdAt: '2024-05-20T03:20:37.915Z',
  };
  ...
  describe('new Notification page', () => {
    ...
    it('should create an instance of Notification', () => {
      cy.get(`[data-cy="type"]`).type('sugary beetle');
      cy.get(`[data-cy="type"]`).should('have.value', 'sugary beetle');

      cy.get(`[data-cy="message"]`).type('psst interlock');
      cy.get(`[data-cy="message"]`).should('have.value', 'psst interlock');

      cy.get(`[data-cy="status"]`).type('when');
      cy.get(`[data-cy="status"]`).should('have.value', 'when');

      cy.get(`[data-cy="createdAt"]`).type('2024-05-20T16:59');
      cy.get(`[data-cy="createdAt"]`).blur();
      cy.get(`[data-cy="createdAt"]`).should('have.value', '2024-05-20T16:59');
